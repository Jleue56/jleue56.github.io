...
class Particle {
  constructor(net) {
    ...
    if (net.opts.edgeOnly) {
      ...
      if (Math.random() <= 0.5) {
        this.x_move = false;
        this.y_move = true;
        if (this.rand <= this.threshold1) {
          // Spawn at the left Side
          this.x = net.opts.sizeMax;
        } else if (this.rand <= this.threshold2) {
          // Spawn at the right side
    ...
    this.s = new TVar(
      net.opts.sizeFlicker,
      net.newParticleSize,
      net.newSizeChangeTime
    );
  ...
  update(rand, wind) {
    if (this.x_move) {
      this.x =
        (this.x + this.vx.update(rand) + wind[0] + this.net.limx) %
        this.net.limx;
    }
    if (this.y_move) {
      this.y =
        (this.y + this.vy.update(rand) + wind[1] + this.net.limy) %
        this.net.limy;
    }
    this.s.update(rand);
  }
...
class ParticleNetwork {
  constructor(elementId, options) {
    ...
    this.particles = [];
    this.numParticles =
      (this.opts.density * (this.limx * this.limy)) / (500.0 * 500.0);
    this.run = false;
  ...
  updateAndDraw() {
    ...
    for (let i = 0; i < this.numParticles; i++) {
      ...
      for (let j = 0; j < this.numParticles; j++) {
        ...
        if (distanceIsh < closest) closest = distanceIsh;
      }
